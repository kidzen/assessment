...
export default async function RoleHasPermissionPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  return (
    <>
      <header className="mt-2 mb-4">
        <Heading>
          Role Has Permission #{roleHasPermission.id.substring(0, 6)}...
        </Heading>
      </header>
      ...
      <section className="relative overflow-hidden rounded-lg border border-gray-200 p-4 sm:p-6 lg:p-8 max-w-xl mb-4">
        ...
        <p className="text-gray-700 mb-4 last:mb-0">
          <strong className="text-gray-900">Role_id:</strong>{" "}
          {roleHasPermission.role_id}
        </p>
        <p className="text-gray-700 mb-4 last:mb-0">
          <strong className="text-gray-900">Permission_id:</strong>{" "}
          {roleHasPermission.permission_id}
        </p>
      </section>

      <footer>
        <Link href="/role_has_permissions" className="underline text-gray-500">
          Return to Role Has Permissions list
        </Link>
      </footer>
    </>
